...
function utilsFactory ( DBG, RSVP ) {
  ...
  var config = {
    error : {
      ...
      action        : {
        ...
        CHANGE_STATE : 3,
        ESCALATE     : 4,
        DESTROY_VIEW : 5, // NOTE : a change of state will have the same effect, keep it to make it semantic?
        THROW_ERR    : 6
      }
    }
  };
      ...
      if (flag_no_transform) {
        ...
        var html_class_attr = aAttributes.reduce(function ( accu, attribute ) {
          ...
          var replace_attr_value = mapAttrClass[attribute][attr_value_in_$el ?
                                                           ...
                                                           undefined];
          // log.debug("attribute name, attribute value, mapping", attribute, attr_value_in_$el, replace_attr_value);
          return [accu,
  ...
  /**
   * Helper function for controller function setting view state for assessing whether the state could be correctly computed
   * @param error_handler_result
   * @returns {boolean} : false if the view state properties could be correctly computed
   */
  function isErrorSettingViewState ( error_handler_result ) {
    return !((error_handler_result && error_handler_result.action === UT.config.error.action.OK) ||
             !error_handler_result)
  }

  function isErrorSettingViewStateEscalate ( error_handler_result ) {
    return error_handler_result.action === UT.config.error.action.ESCALATE
  }
  ...
  var _UT =
  {
    ...
    isErrorSettingViewState         : isErrorSettingViewState,
    isErrorSettingViewStateEscalate : isErrorSettingViewStateEscalate
  };
