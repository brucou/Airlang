...
SIO.sio_onHighlight_stop_words = function sio_onHighlight_stop_words ( msg, callback ) {
   console.log('sio_onHighlight_stop_words message received: ', msg);
   ...
   var stop_word_list = DB.get_stop_words('cs');
   var qryHighlight_stop_words =
          "select ts_headline('cs', $1, to_tsquery('cs', $2), " +
          "'StartSel=\"<span class = ''highlight''>\", StopSel=\"</span>\", HighlightAll=true, ShortWord=0') " +
          "as highlit_text"; //important the first %s has no quotes

   pgClient.query(
      qryHighlight_stop_words, [msg, stop_word_list],
      function ( err, result ) {
         SIO.process_qryHighlight_stop_words(err, result, callback);
      });

   LOG.write(LOG.TAG.INFO, 'query sent to database server, waiting for callback');
};

SIO.process_qryHighlight_stop_words = function process_qryHighlight_stop_words ( err, result, callback ) {
   //LOG.entry('process_qryHighlightImportantWords');
   if (err) {
      LOG.write(LOG.TAG.ERROR, 'error running qryHighlight_stop_words query', err);
      callback(err, result);
      //LOG.exit('process_qryHighlightImportantWords');
      return;
   }
...
SIO.init_listeners = function init_listeners () {
   ...
   io.of(RPC_NAMESPACE).on(
      'connect', function ( socket ) {
         ...
         socket.on('highlight_stop_words', SIO.sio_onHighlight_stop_words);
